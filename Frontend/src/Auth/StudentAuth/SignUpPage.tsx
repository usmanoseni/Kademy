...
const LoginPage: React.FC <StudentSignUpProps > = ({ theme }) => { 
    ...
    try {
        ...
        const res = await StudentAuth.register({name: form.name, email: form.email.trim(), password: form.password});
        if (res.user) {
    ...
    return (
        <div className={`flex-col justify-center items-center w-full  h-full ${theme === 'dark' ? 'bg-slate-900' : 'bg-radial-[at_ 25%_50%_75%] from-white via-blue-100/10 to-indigo-300/20 to-90%'}`}>
            <div className={` ${theme === 'dark' ? 'pattern' :'pattern'  } w-full h-full  flex-col justify-center items-center gap-10 flex relative`} >
                ...
                <div className='bg-white flex z-30 rounded-3xl 4/5 lg:w-3/5 my-14 mx-8 md:my-13 lg:my-15 lg:h-[42.8rem] overflow-hidden dark:bg-slate-900/90 shadow-2xl shadow-slate-200 dark:shadow-slate-800'>
                    <div className='relative hidden lg:flex w-1/2  rounded-l-3xl'>
                        ...
                        <div className='absolute inset-0 flex items-center justify-center p-6 text-center text-white'>
                            <div>
                                ...
                                <p className='mt-2 text-sm text-blue-100'>Start your learning journey with Kademy.</p>
                            </div>
                        </div>
                    </div>
                    <div className='flex-1 p-6'>
                        ...
                        <div className='flex justify-between items-center mb-4 text-xs'>
                            ...
                            <Link to='/auth/student/login'><button className='ml-2 text-indigo-500  dark:text-indigo-400 dark:hover:text-indigo-300 hover:bg-indigo-100/70 hover:border-transparent px-3 py-1.5 cursor-pointer border border-indigo-400/70 rounded-md'>Login</button></Link>
                        </div>
                        <div>
                            <h3 className='text-2xl font-bold text-blue-950 dark:text-white'>Sign In</h3>
                            <p className='mt-2 text-sm text-gray-600 dark:text-gray-300'>Register to access lessons and study materials for your academic excellent.</p>
                        </div>
                        <div>
                            {error && (
                            ...
                            <form onSubmit={submit}  className='mt-4'>
                                ...
                                <label className='' >
                                    <span className="after:content-['*'] dark:text-slate-200 after:text-red-400 after:ml-0.5 text-sm font-medium   text-gray-800 datk:text-gray-300">Comfirm password</span>
                                    <div className="relative ">
                                        <input value="password" onChange={update} name="password" type={password ? "text" : "password"} placeholder="Confirm password" className="invalid:outline-pink-500 invalid:text-pink-600 focus:invalid:outline-pink-500 text-sm disabled:border-gray-200 disabled:bg-gray-50 disabled:text-gray-500 mt-1 :placeholder:text-gray-400 px-2  py-2 pl-10 font-medium text-gray-800 block w-full rounded-sm invisible:outline-1 outline-1 outline-slate-400/60 dark:bg-gray-800 dark:text-white dark:outline-gray-600 dark:focus:outline-indigo-400 focus:outline-indigo-500 shadow-sm " />
                                        <div className="absolute inset-y-0 left-0 flex items-center px-1.5 pointer-events-none bg-slate-100 justify-center rounded-l-sm dark:bg-gray-800">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-5 text-gray-500 dark:text-gray-300">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" />
                                            </svg>
                                        </div>
                                        <div onClick={() => setConfirmPassword(!confirmPassword)} className="absolute inset-y-0 right-0 flex items-center px-1.5 cursor-pointer justify-center rounded-l-sm ">
                                            { !confirmPassword ?(<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"  className="size-5 text-gray-500 dark:text-gray-300 hover:text-indigo-600">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                            </svg>)
                                                : (
